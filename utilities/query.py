...
def prepocess_query(user_query:str):
    preprocessed_query = re.sub(
        r"\s+",
        " ",
        user_query.lower().replace('"', '')
    )
    ...
    return " ".join(
        [
            stemmer.stem(word) 
            for word in preprocessed_query_tokens
        ]
    )


def classify_query(user_query:str, threshold:float=0.5, top_k:int=20):
    preprocessed_query = prepocess_query(
        user_query
    )
    classes_, probas_ = QUERY_CLASSIFIER.predict(
        user_query,
        k=top_k
    )    
    relevant_classes = []
    cum_proba = 0.0
    for (class_, proba_) in zip(classes_, probas_):
        if cum_proba >= threshold:
...
def search(
    ...
    threshold = 0.5
    query_classes, cum_proba = classify_query(user_query, threshold=threshold)

    #### W3: create filters and boosts
    filter_expression = " OR ".join(query_classes).strip(" OR ")
    filters = [
        {"term": { "categoryLeaf": f"{filter_expression}" } }
    ] if (query_classes and cum_proba > threshold) else None

    print(
        f"Query classes: {repr(query_classes)}",
        f"Cum proba: {cum_proba}"
    )
    print(
        "filters",
        filters
