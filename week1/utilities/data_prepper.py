...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        # Loop over the hits structure returned by running `log_query` and then extract out the features 
        #from the response per query_id and doc id.  Also capture and return all query/doc pairs that didn't return features
        ...
        if response and len(response['hits']) > 0:
            for hit in response['hits']['hits']:
                ...
                for log_entry in log_entries:
                    if not (log_entry['name'] in feature_results):
                        feature_results[log_entry['name']] = [log_entry.get('value', 0)]
                    else:
